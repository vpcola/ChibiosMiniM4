...
#if GFILE_NEED_DISKINFO && _FS_MINIMIZE <= 1
static unsigned long long fatfsGetDiskSize(const char * drive);
static unsigned long long fatfsGetDiskFree(const char * drive);
static unsigned int fatfsGetDiskClusters(const char* drive);
static unsigned int fatfsGetDiskClusterSize(const char* drive);
#endif
...
const GFILEVMT FsFatFSVMT = {
	...
	#if GFILE_NEED_DISKINFO 
		#if _FS_MINIMIZE <= 1
		fatfsGetDiskSize, fatfsGetDiskFree, fatfsGetDiskClusters, fatfsGetDiskClusterSize,
		#else
		0, 0, 0, 0,
		#endif
	#endif
...
#if GFILE_NEED_DISKINFO && _FS_MINIMIZE <= 1
unsigned long long fatfsGetDiskFree(const char * drive)
{
	(void)drive;
	FRESULT ferr;
	uint32_t clusters;
	...
	if (fatfs_mounted)
	{
		ferr = f_getfree(drive, &clusters, &fsp);
		if (ferr != FR_OK)
			return 0;
		// return clusters * (uint32_t)fatfs_fs.csize * (uint32_t) fatfs_fs.ssize ;
		if (fsp)
		  return clusters * (uint32_t)fsp->csize * MMCSD_BLOCK_SIZE;
	}

	return 0;
}

unsigned long long fatfsGetDiskSize(const char* drive)
{
	(void)drive;

	if (fatfs_mounted)
		// return (uint32_t) fatfs_fs.last_clust * (uint32_t)fatfs_fs.csize * (uint32_t) fatfs_fs.ssize ;
		return (uint32_t) fatfs_fs.last_clust * (uint32_t)fatfs_fs.csize * MMCSD_BLOCK_SIZE;

	return 0;
}

unsigned int fatfsGetDiskClusters(const char* drive)
{
  (void)drive;
  FRESULT ferr;
  uint32_t clusters;
...
unsigned int fatfsGetDiskClusterSize(const char* drive)
{
  (void)drive;

  if (fatfs_mounted)
    return fatfs_fs.csize;
